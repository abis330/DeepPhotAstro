...
def get_model(X, Y, size=80):
    ...
    rnn = CuDNNLSTM(size, return_sequences=False)(hist)
...
def evaluate(fold_idx, model, x, y, datasetType):
    ...
    plt.savefig('{}{}_Fold_{}_confusion_matrix.png'.format(utils.result_dir_path,
    datasetType, fold_idx))
    ...
    true_and_preds_df.to_csv('{}lstm_{}_fold_{}_true_and_pred.csv'.format(utils.result_dir_path,
                                                                          datasetType, fold_idx), index=False)
